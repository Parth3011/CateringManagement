...
export default function Profile({ user, onUpdateProfile }) {
  ...
  const handleSubmit = (event) => {
    ...
    try {
      axios.put("http://localhost:7000/api/updateprofile", inputdata)
        .then((resp) => {
  ...
  return (
    <div style={{ height: '150vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'linear-gradient(to bottom right, #c3d1e4, #e5e7eb)' }}>
      <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', padding: '20px', borderRadius: '10px', boxShadow: '0px 0px 10px rgba(0, 0, 0, 0.1)', width: "800px" }}>
        <h1 style={{ textAlign: 'center', marginBottom: '20px' , fontSize:"50px" , textDecoration:"underline" }}>Profile</h1>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <input type="text" style={{ marginBottom: '10px', padding: '8px', borderRadius: '5px', border: '1px solid #ccc' }}  name="email" value={inputdata.email} onChange={handledata} disabled />
